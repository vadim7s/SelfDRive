...
"""
Vadim changes:
1. simplifies render of multiple sensors to just one RGB cam (done)
2. add steering and throttle values to the screen (done)
3. Change town to 05 (done)
4. Initiate a random yaw rotation at spawning (done) - may add text to top screen to show 
5. Limit to freeway (done)
6. Put timer clock on display (done and then removed as it showed multiple episodes timer)
7. Limit simulation to X seconds (done, but better stop the car and introduce yaw factor)
8. Create loop for new episodes (done but it hangs the server)
9. Save images and control inputs to disk (done, but some are coorupted)
New direction: make existing car slow down and get random yaw angle
10. Identify frames at spawning which need to be ignored (done - first 3 ticks are ignored)
11. Capture controls coming from autopilot - they need to become labels for each frame image (done - steering inputs saved in separate files)
12. Introduce yaw change every so often (done)
13. Limit speed to 10 m/sec (done)
14. Do not change yaw at speed below 2 kmh (done)
13. Filter/remove/not-save images when the car is stopped or turning at an itersection:
    13.1 Images at speed below 2 kmh are not captured (done)
    13.2 Images within certain proximity to junctions/intersections (done)
14. Change random yaw to X frames after steering went straight
